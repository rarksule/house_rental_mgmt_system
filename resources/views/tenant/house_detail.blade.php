<x-app-layout>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-4 text-md-end">
                    <h2 class="text-primary display-6">{{ $house->price }}<small
                            class="text-muted">{{ __('message.per') }}</small></h2>
                    <div class="mt-3">
                ...
                <form action="{{ route('tenant.rate') }}" method="post">
                    ...
                    <div class="rating-container mb-4">
                        ...
                        <button type="submit" class="btn btn-primary"
                            id="submit-rating">{{ __('message.submit_review') }}</button>
                    </div>
                                            ...
                                            <div class="card bg-light mb-2">
                                                <div class="card-body p-3">
                                                    <div class="d-flex justify-content-between mb-1">
                                                        ...
                                                        <small
                                                            class="text-muted">{{ timeAgo($reply->created_at) }}</small>
                                                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ Auth::check() ? route('messagesSend') : route('login') }}"
                            ...
                            <div class="my-3">
                                ...
                                <textarea class="form-control" name="content" placeholder="Type your message..." required id="message"
                                    rows="3"></textarea>
                            </div>
    ...
    @push('script')
        <script>
            ...
            const tourRequested = " {{ tourRequested()  ? 1 : 0}}";
            const isTenant = "{{ isTenant() }}";
            
            document.getElementById('req-tour').addEventListener('click', function(e) {
                
                e.preventDefault();
                if (!isTenant || tourRequested==1) {
                    return;
                }
                console.log(tourRequested)
